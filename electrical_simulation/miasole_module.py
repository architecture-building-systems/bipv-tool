...
def partial_shading(irrad_on_subcells, temperature=25, irrad_temp_lookup_df=None, module_database_path=None,
                    module_name='MiaSole_Flex_01_70N', numcells=44, n_bypass_diodes=22, num_subcells=5):
    ...
    breakdown_voltage = -6.1  # Von der Zelle oder vom Modul?
    ...
    for sub_cell_irrad in irrad_on_subcells:
        ...
        if not(value_from_table!=value_from_table):  # Checks for NaNs
            ...
            subcell_v_values.append(subcell_voltage)
            ...
            subcell_current, subcell_voltage = connect.clean_curve([subcell_current, subcell_voltage],0.01)
    ...
    #with bypass diode:
        ...
        for substring in range(n_bypass_diodes-1):
            i_module, v_module = connect.series_connect(i_module, v_module, i_connected[substring+1],
                                                        v_connected[substring+1]) #Includes a clean curve
